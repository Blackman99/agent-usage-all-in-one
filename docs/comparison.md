...
>
> **Fact-check 2026-09-10:** Axes match README (macOS-first, local loopback, providers Codex/Claude Code/OpenCode/Grok/dsh/Antigravity, advice-only, API retail ≠ bill, Grok Build/SuperGrok vs xAI API never summed, Node ≥ 24). Named competitors added in companion `named-competitors.md` (ccusage / CodeBurn / Token Monitor / TokenTracker) after 开源对标 scan — P0: no Cursor yet, macOS only, don’t oversell all-in-one.
...
*Update when provider coverage or competitor facts change.*
