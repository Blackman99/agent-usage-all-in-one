...
| OpenCode Go | `1.18.23`                             | Local executable, official account endpoint, and local export smoke were re-exercised read-only on 2026-08-28 | Account quota plus this-Mac model/day history; output and separate reasoning semantics validated without exposing private totals                                 | live read-only passed              |
...
- OpenCode Go rendered official account quota separately from this-Mac model/day
  Token history and its Provider-reported estimate, with source-native coverage.
